...
int main(int argc, char *argv[]) {
	...
	//Máximo de 3 peticiones encoladas atendidas por el servidor
	...
	for (i=0; i<5; i++){
		...
		//y el servidor usará para devolver la informacion pedida.
		...
		//ret te dice la cantidad de bytes del mensaje, el tamaño.
		ret = read(sock_conn, peticion, sizeof(peticion));
		printf("Recibido\n");
		
		//Tenemos que añadirle la marca de fin de string
		//para que no escriba lo que hay después en el buffer
		peticion[ret]='\0';
		...
		// Dependiendo del cÃ³digo, realizamos diferentes consultas
		if (codigo == 1) {
			// Consultar nombre
...
void consultar_password(char* id_usuario, char* respuesta) {
	...
	if ((row = mysql_fetch_row(res))) {
		sprintf(respuesta, "ContraseÃ±a: %s", row[0]);
	} else {
		strcpy(respuesta, "Usuario no encontrado");
	}
